...
declare actor uuid:=auth.uid(); e sontu_private.event_instances; t sontu_private.event_seating_tables; participant_row sontu_private.event_participants; clean_label text:=btrim(coalesce(table_label,'')); clean_name text:=btrim(coalesce(attendee_name,'')); assigned integer;
begin
  ...
  if action='UPSERT_TABLE' then
    ...
    select * into t from sontu_private.event_seating_tables where id=table_id and event_instance_id=e.id for update;
    select * into participant_row from sontu_private.event_participants where id=participant_id and event_instance_id=e.id and commitment_state='CONFIRMED';
    ...
    delete from sontu_private.event_seating_assignments where event_participant_id=participant_row.id and attendee_name=clean_name;
    insert into sontu_private.event_seating_assignments(event_instance_id,seating_table_id,event_participant_id,attendee_name) values(e.id,t.id,participant_row.id,clean_name);
  ...
  if action<>'READ' then
    insert into sontu_private.audit_entries(event_instance_id,operation_id,actor_ref,audit_kind,metadata)
      values(e.id,gen_random_uuid(),actor::text,'event_seating_'||lower(action),jsonb_build_object('table_id',table_id,'participant_id',participant_id,'attendee_name',clean_name));
  end if;
  return jsonb_build_object('status','ready','tables',coalesce((select jsonb_agg(jsonb_build_object('id',st.id,'label',st.label,'capacity',st.capacity,'assigned_count',(select count(*) from sontu_private.event_seating_assignments a where a.seating_table_id=st.id),'assignments',coalesce((select jsonb_agg(jsonb_build_object('participant_id',a.event_participant_id,'attendee_name',a.attendee_name,'invitee_name',ep.display_name) order by a.attendee_name) from sontu_private.event_seating_assignments a join sontu_private.event_participants ep on ep.id=a.event_participant_id where a.seating_table_id=st.id),'[]'::jsonb)) order by st.label) from sontu_private.event_seating_tables st where st.event_instance_id=e.id),'[]'::jsonb));
end $$;
