-- Explicit event-day transition. Starting an event is host-authorized,
-- idempotent, and recorded independently from publishing or closeout.
create function sontu_private.start_event(event_id uuid,operation_id uuid) returns jsonb
language plpgsql security definer set search_path='' as $$
declare actor uuid:=auth.uid(); e sontu_private.event_instances; prior sontu_private.event_operations_requests; fp text; result jsonb;
begin
 ...
 fp:=jsonb_build_object('event_id',event_id,'action','START_EVENT')::text;
 select * into prior from sontu_private.event_operations_requests r where r.operation_id=start_event.operation_id;
 if found then
  if prior.actor_user_id<>actor or prior.fingerprint<>fp then return sontu_private.fail('IDEMPOTENCY_MISMATCH'); end if;
  return prior.result;
 end if;
 ...
 if e.lifecycle='IN_PROGRESS' then
  result:=jsonb_build_object('status','ready','event_id',event_id,'lifecycle','IN_PROGRESS');
 elsif e.lifecycle<>'PUBLISHED' then
  return sontu_private.fail('INVALID_STATE');
 else
  update sontu_private.event_instances set lifecycle='IN_PROGRESS',updated_at=now() where id=event_id;
  result:=jsonb_build_object('status','ready','event_id',event_id,'lifecycle','IN_PROGRESS');
  insert into sontu_private.audit_entries(event_instance_id,operation_id,actor_ref,audit_kind,metadata)
    values(event_id,operation_id,actor::text,'EVENT_STARTED',jsonb_build_object('from_lifecycle','PUBLISHED'));
 end if;
...
revoke all on function sontu_private.start_event(uuid,uuid) from public,anon,authenticated;
grant execute on function sontu_private.start_event(uuid,uuid) to authenticated;
create function public.sontu_start_event(event_id uuid,operation_id uuid) returns jsonb
language sql security invoker set search_path='' as $$ select sontu_private.start_event(event_id,operation_id) $$;
revoke all on function public.sontu_start_event(uuid,uuid) from public,anon,authenticated;
grant execute on function public.sontu_start_event(uuid,uuid) to authenticated;

-- Admission opens only once the host has explicitly started the event.
...
begin
 ...
 elsif e.lifecycle<>'IN_PROGRESS' or p.commitment_state<>'CONFIRMED' then result:=jsonb_build_object('status','error','error_code','NOT_ELIGIBLE','result','INVALID');
