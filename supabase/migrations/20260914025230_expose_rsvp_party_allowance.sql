...
begin
 ...
 'participants',coalesce((select jsonb_agg(jsonb_build_object('id',p.id,'display_name',p.display_name,'commitment_state',p.commitment_state,'invitation_state',p.invitation_state,'invitation_email',p.invitation_email,'link_revoked',p.token_revoked_at is not null,'plus_one_allowance',p.plus_one_allowance,'response',r.state) order by p.display_name) from sontu_private.event_participants p left join sontu_private.participant_reconfirmations r on r.event_participant_id=p.id and r.consequence_case_id=cid where p.event_instance_id=e.id),'[]'),
