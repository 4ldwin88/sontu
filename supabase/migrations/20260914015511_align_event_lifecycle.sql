-- Align the clean beta database to the durable product model.
alter table sontu_private.event_instances
  drop constraint event_instances_lifecycle_check;

alter table sontu_private.event_instances
  add constraint event_instances_lifecycle_check
  check (lifecycle in ('DRAFT','PUBLISHED','IN_PROGRESS','CANCELLED','COMPLETED'));
...
declare actor uuid:=auth.uid(); e sontu_private.event_instances; prior sontu_private.event_operations_requests; result jsonb; confirmed_n integer; admitted_n integer; declined_n integer; blockers integer;
begin
 ...
 if e.lifecycle not in ('PUBLISHED','IN_PROGRESS') then return jsonb_build_object('status','error','error_code','INVALID_STATE'); end if;
...
language sql stable security definer set search_path='' as $$
  ...
  from (
    ...
    where e.event_kind='SIMPLE' and e.visibility='PUBLIC' and e.lifecycle in ('PUBLISHED','IN_PROGRESS','CANCELLED','COMPLETED')
