alter table sontu_private.event_versions
  drop constraint if exists event_versions_cover_key_check;

alter table sontu_private.event_versions
  add constraint event_versions_cover_key_check check (
    cover_key in ('food','music','market','yoga','sailing','sunset','none')
    or cover_key ~ '^upload:[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/[-_.a-zA-Z0-9]+[.](jpg|jpeg|png|gif|webp)$'
  );
...
begin
  if to_regclass('storage.buckets') is not null and to_regclass('storage.objects') is not null then
    ...
    execute 'drop policy if exists "Event media public read" on storage.objects';
    execute 'drop policy if exists "Event media own folder insert" on storage.objects';
    execute 'drop policy if exists "Event media own folder update" on storage.objects';
    execute 'drop policy if exists "Event media own folder delete" on storage.objects';

    execute 'create policy "Event media public read" on storage.objects for select to anon, authenticated using (bucket_id = ''event-media'')';
    execute 'create policy "Event media own folder insert" on storage.objects for insert to authenticated with check (bucket_id = ''event-media'' and owner = (select auth.uid()) and (storage.foldername(name))[1] = (select auth.uid())::text)';
    execute 'create policy "Event media own folder update" on storage.objects for update to authenticated using (bucket_id = ''event-media'' and owner = (select auth.uid()) and (storage.foldername(name))[1] = (select auth.uid())::text) with check (bucket_id = ''event-media'' and owner = (select auth.uid()) and (storage.foldername(name))[1] = (select auth.uid())::text)';
    execute 'create policy "Event media own folder delete" on storage.objects for delete to authenticated using (bucket_id = ''event-media'' and owner = (select auth.uid()) and (storage.foldername(name))[1] = (select auth.uid())::text)';
  end if;
end
$storage$;

create or replace function sontu_private.change_event_cover(
  event_id uuid,
...
begin
  ...
  if cover_key not in ('food','music','market','yoga','sailing','sunset')
    and cover_key !~ ('^upload:' || actor::text || '/' || event_row.id::text || '/[-_.a-zA-Z0-9]+[.](jpg|jpeg|png|gif|webp)$') then
...
create or replace function public.sontu_change_event_cover(event_id uuid,expected_version integer,cover_key text,operation_id uuid)
