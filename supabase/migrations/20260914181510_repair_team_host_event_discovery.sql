-- Preserve authenticated participant identity and notification state while
-- adding only the contextual host-team access from the preceding migration.
...
begin
 ...
 return jsonb_build_object('status','ready','events',coalesce((select jsonb_agg(x order by x.starts_at nulls last,x.id) from (
  select e.id,e.lifecycle,e.event_category as category,e.event_format as format,
   ...
   sontu_private.can_manage_event_state(e.id,auth.uid()) as hosting,p.commitment_state,p.invitation_state,
  ...
  where e.event_kind='SIMPLE' and (sontu_private.can_manage_event_state(e.id,auth.uid()) or (p.id is not null and e.lifecycle<>'DRAFT'))
 ) x),'[]'::jsonb));
end $$;
...
revoke all on function sontu_private.my_events(),sontu_private.event_hub(uuid) from public,anon,authenticated;
