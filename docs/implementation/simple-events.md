...
## Next hosting slice — founder authorized
The founder accepted own-account registration and asked to continue hosting. Published-event time edits now reuse the creation flow's event-timezone conversion and DST ambiguity rejection, retain the server version/idempotency command boundary, and reject start times at or after the existing end. Guest overview counts consume the existing host projection and distinguish committed attendance, pending active invitations, and declined/withdrawn relationships. No new database or messaging authority is activated. Signup-confirmation removal remains pending the hosted configuration capability and independent private-invitation email-assurance guard described in account-portal.md.

Founder review correction: Upcoming includes published events the actor hosts as well as confirmed participation. Hosting remains the operational view and includes drafts/history. Host inclusion does not create participation or reserve a place; the card retains its host label. Cancelled/closed/draft events are excluded from Upcoming. Creation browser coverage now verifies the same published identity appears in both Upcoming and Hosting.

## Host workflow completion slice
Founder authorized larger autonomous slices after Upcoming review. Recovered current UX host-workspace/creation specifications and preserved email-delivery deferral. The authorized My Events projection now includes description/end time under its unchanged owner/verified-recipient predicate. Hosting groups drafts, published/upcoming, in-progress, and history/cancelled. Ended events leave Upcoming. Host labels follow the actual hosting relationship in every view. Event Hub shows current description, start/end, timezone, location, lifecycle and management entry; host workspace links back to that representation. Creation and workspace return to contextual Hosting. Guest search/status filters consume the existing projection; no identity lookup or new permission model. Time/cancellation review identifies event and participant impact and explicitly states email delivery is not connected. Cancellation remains authoritative and removes Upcoming presence without deleting history or implying communication/obligation settlement. Browser coverage extends publication through current-detail editing, guest filtering and cancellation/history across all widths.
